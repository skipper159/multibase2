...
import { getExpectedSharedPorts, resolveSharedServicePorts } from '../utils/sharedPorts';
...
export function createSharedRoutes(
  ...
  router.get('/status', async (_req: Request, res: Response) => {
    try {
      ...
      const expectedPorts = getExpectedSharedPorts(sharedEnv || {}, getSharedDir());
      ...
      const servicesWithPorts = services.map(service => ({
        ...service,
        ports: resolveSharedServicePorts(service.name, service.portBindings, expectedPorts),
      }));
      const actualHostPort = (serviceName: string, container: number) =>
        servicesWithPorts
          .find(service => service.name === serviceName)
          ?.ports?.find(port => port.actual && port.container === container)?.host;
      const ports = {
        postgres: actualHostPort('db', 5432),
        studio: actualHostPort('studio', 3000),
        analytics: actualHostPort('analytics', 4000),
        pooler: actualHostPort('pooler', 6543),
        gateway: actualHostPort('nginx-gateway', 8000),
        meta: actualHostPort('meta', 8080),
      };
    ...
    async (_req: Request, res: Response) => {
      try {
        ...
        const { stdout, stderr } = await runDockerCommand(
          ['compose', ...getSharedComposeArgs(sharedDir), 'up', '-d'],
          { cwd: sharedDir }
        );
    ...
    async (_req: Request, res: Response) => {
      try {
        ...
        const { stdout } = await runDockerCommand(
          ['compose', ...getSharedComposeArgs(sharedDir), 'down'],
          { cwd: sharedDir }
        );
   ...
   * Uses docker exec over the configured direct local Docker socket.
   */
  ...
  router.post(
    '/databases',
    requireAdmin,
    auditLog('SHARED_DATABASE_CREATE', {
      includeBody: true,
      getResource: req => req.body?.projectName || 'unknown',
    }),
    async (req: Request, res: Response) => {
      try {
        ...
        if (!dbName) {
          res
            .status(400)
            .json({
              error: 'projectName must contain only lowercase letters, numbers, and single hyphens',
            });
          return;
        }
  ...
  router.delete(
    '/databases/:name',
    requireAdmin,
    auditLog('SHARED_DATABASE_DROP', { getResource: req => req.params.name }),
    async (req: Request, res: Response) => {
      try {
